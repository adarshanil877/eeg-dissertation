...
clustering_folder = "combined_clustering_results"
...
print("\nRUNNING COMBINED CLUSTERING FOR ALL PARTICIPANTS")

#STORING ALL PARTICIPANT DATA
all_X = []
all_y = []
all_participants = []
all_epochs = []
...
for file in files:
    ...
    all_X.append(X)
    all_y.append(y)
    all_participants.extend([participant] * len(y))
    all_epochs.extend(range(len(y)))

#COMBINING ALL PARTICIPANTS
X_all = np.vstack(all_X)
y_all = np.concatenate(all_y)

print("\nCOMBINED FEATURE SHAPE :", X_all.shape)
print("TOTAL EPOCHS :", len(y_all))
...
plt.xlabel("PCA Component 1")
plt.ylabel("PCA Component 2")
plt.title("All Participants EEG Feature Clustering")
plt.legend()
plt.grid(alpha=0.3)
plt.tight_layout()
...
graph_path = os.path.join(
    clustering_folder,
    "all_participants_clustering.png"
)
